...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		switch (transaction->Source) {
			...
			case Transaction_Source_Type::CACHE:
				...
				if (_my_instance->block_manager->Block_has_ongoing_gc_wl(transaction->Address)) {  /* return plane_record->Blocks[block_address.BlockID].Has_ongoing_gc_wl; */
					if (_my_instance->block_manager->Can_execute_gc_wl(transaction->Address)) {  /* Ongoing_user_program_count + Ongoing_user_read_count == 0 */
						...
						if (block->Current_page_write_index - block->Invalid_page_count > 0) {  /* 还有有效页面 需要进行移动 */
							...
							for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
								if (_my_instance->block_manager->Is_page_valid(block, pageID)) {  /* (block->Invalid_page_bitmap[page_id / 64] & (((uint64_t)1) << page_id)) == 0 */
	...
	void GC_and_WL_Unit_Base::run_static_wearleveling(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		block_manager->GC_WL_started(wl_candidate_block_id);  /* plane_record->Blocks[block_address.BlockID].Has_ongoing_gc_wl = true; */
