...
namespace SSD_Components
{
	...
	*/
	...
	private:
		unsigned int* stream_count_per_priority_class;  /* 每个priority_class中包含有几个stream */
		stream_id_type** stream_ids_per_priority_class;  /* 为每个priority_class赋值相应stream数量数组 stream_ids_per_priority_class[i] = new stream_id_type[stream_count_per_priority_class[i]]; */
		FLIN_Flow_Monitoring_Unit**** flow_activity_info;  /* [channel_id][chip_id][pclass_id][stream_cntr] 注意内部元素是FLIN_Flow_Monitoring_Unit */
		sim_time_type flow_classification_epoch;  /* 建立Register_sim_event的时间开销 */
		unsigned int alpha_read_for_epoch, alpha_write_for_epoch;  /* alpha_read / (channel_count * chip_no_per_channel) / flash_page_size */
		unsigned int no_of_priority_classes;
		double F_thr;//Fairness threshold for high intensity flows, as described in Alg 1 of the FLIN paper (0.6)
		std::set<stream_id_type> ***low_intensity_class_read, *** low_intensity_class_write; // /* [channel_id][chip_id] set<stream_id_type>[no_of_priority_classes] */ As described in Alg 1 of the FLIN paper
		...
		Flash_Transaction_Queue*** UserReadTRQueue;  /* [channel_id][chip_id][pclass_id] */
		NVM_Transaction_Flash_RD**** Read_slot;  /* Read_slot[channel_id][chip_id] = new NVM_Transaction_Flash_RD*[no_of_priority_classes]; */
