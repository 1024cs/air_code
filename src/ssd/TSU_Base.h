...
public:
	...
	void Prepare_for_transaction_submit()
	{
		...
		{
			// qq("src/ssd/TSU_Base.h void Prepare_for_transaction_submit opened_scheduling_reqs > 1");
			return;
		}
		// // if (opened_scheduling_reqs >= 2)
		// // 	qq(opened_scheduling_reqs);
		transaction_receive_slots.clear();
	}

	void Submit_transaction(NVM_Transaction_Flash *transaction)   /* mapping提交到TSU模块的入口（可能还有GC等其他的模块会调用） */
	{
		/* zy1024cs */
		transaction->tsu_time = Simulator->Time();
	...
	unsigned int Get_busy_channels_number(){
        unsigned int busy_number = 0;
	...
	unsigned int Get_channel_count() {return channel_count;}
...
protected:
	...
	static void handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash *transaction);  /* 放到ONIF中 但是是个空函数 */
	static void handle_channel_idle_signal(flash_channel_ID_type);  /* 放到ONIF中 一旦发现有channel空闲就可以调用 即通知TSU可以开始处理队列中新的事务请求 */
	static void handle_chip_idle_signal(NVM::FlashMemory::Flash_Chip *chip);  /* 放到ONIF中 */
	int opened_scheduling_reqs;
	void process_chip_requests(NVM::FlashMemory::Flash_Chip* chip) /* 按照read->write->erase的优先级 */
	{
