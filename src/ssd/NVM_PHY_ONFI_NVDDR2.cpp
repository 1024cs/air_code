...
namespace SSD_Components {
	...
	void NVM_PHY_ONFI_NVDDR2::Setup_triggers()
	{
		...
		for (unsigned int i = 0; i < channel_count; i++) {
			for (unsigned int j = 0; j < chip_no_per_channel; j++) {
				channels[i]->Chips[j]->Connect_to_chip_ready_signal(handle_ready_signal_from_chip);  /* chip层面完成的广播 */
			}
		}
	}
	...
	void NVM_PHY_ONFI_NVDDR2::Send_command_to_chip(std::list<NVM_Transaction_Flash*>& transaction_list)   /* 接收来自TSU调度后的事务列表进入闪存控制器模块 */
	{
		/* zy1024cs */
		for (std::list<NVM_Transaction_Flash*>::const_iterator it = transaction_list.begin(); it != transaction_list.end(); it++){
			(*it)->flash_time = Simulator->Time();
			switch ((*it)->Type) {
				...
				case Transaction_Type::ERASE:
					continue;
				default:
					throw std::invalid_argument("NVM_PHY_ONFI_NVDDR2: Unhandled event specified! zy1024cs");

			}
		}


		/* transaction_list内部事务数量取决于一个die中plane的数量 因为在TSU部分已经将transaction_list内部的事务聚集为其中一个channel的chip的die了 */
		...
		* then something illegarl is happening*/
		...
		if (!dieBKE->Free) {  /* 当前die处于active状态 可能需要Suspend */
		...
		for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
			...
			dieBKE->ActiveTransactions.push_back(*it);  /* 在TSU侧已经保证了transaction_list全都是同一个die的事务 实现die的并行性 */
			dieBKE->ActiveCommand->Address.push_back((*it)->Address);  /* dieBKE->ActiveCommand内部的Address来源于事务的地址 */
		...
		switch (transaction_list.front()->Type) {
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
						it != transaction_list.end(); it++) {   /* 命令和写数据在channel内的传输时间 */
					...
					if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
						...
						chipBKE->Status = ChipStatus::CMD_DATA_IN;    /* 下面的Last_transfer_finish_time包括暂停当前活动时间+命令在chip的传输时间+写数据的传输时间 */
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		if (WaitingCopybackWrites[channel_id].size() > 0) {
			...
			targetChip->StartCMDXfer();  /* this->lastTransferStart = Simulator->Time(); */
		...
		broadcastChannelIdleSignal(channel_id);  /* 只有TSU */
	}
	...
	{   /* 会被丢到chip层面的广播函数中 chip在finish_command_execution时候结尾调用 */
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				chip->StartCMDXfer();  /* this->lastTransferStart = Simulator->Time(); */
				chipBKE->Status = ChipStatus::CMD_IN;
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
				...
				it != dieBKE->ActiveTransactions.end(); it++, i++)
			{
				...
				_my_instance->broadcastTransactionServicedSignal(*it);  /* 包含mapping GC cache 和 TSU */
			}
			...
			if (chipBKE->Status == ChipStatus::IDLE)
				if (chipBKE->HasSuspend)
					_my_instance->send_resume_command_to_chip(chip, chipBKE);  /* 回归到未Suspend的状态 相关状态置为默认值 如Suspended = false; */
			break;
		}
		...
		if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			_my_instance->broadcastChannelIdleSignal(chip->ChannelID);  /* 只有TSU */
		else if (chipBKE->Status == ChipStatus::IDLE)
			_my_instance->broadcastChipIdleSignal(chip);  /* 只有TSU */
	}
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		channels[tr->Address.ChannelID]->Chips[tr->Address.ChipID]->StartDataOutXfer();  /* this->lastTransferStart = Simulator->Time(); */
		...
		switch (bookKeepingEntry->ActiveTransactions.front()->Type)
		{
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->RelatedRead == NULL) {
					chip->StartCMDDataInXfer();  /* this->lastTransferStart = Simulator->Time(); */
					...
					Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime, this,
						bookKeepingEntry, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED);   /* 应该是想先把读搞完 避免出现读到错误的数据 */
				}
				break;
			case Transaction_Type::ERASE:
				chip->StartCMDXfer();
	...
	{   /* 回归到未Suspend的状态 相关状态置为默认值 如Suspended = false; */
