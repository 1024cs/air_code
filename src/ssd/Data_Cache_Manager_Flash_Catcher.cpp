...
#include "Data_Cache_Manager_Flash_Catcher.h"
...
{
	Data_Cache_Manager_Flash_Catcher::Data_Cache_Manager_Flash_Catcher(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
	...
	Data_Cache_Manager_Flash_Catcher::~Data_Cache_Manager_Flash_Catcher()
	{
		switch (sharing_mode)
		{
			case SSD_Components::Cache_Sharing_Mode::SHARED:
	...
	void Data_Cache_Manager_Flash_Catcher::Setup_triggers()
	{
		Data_Cache_Manager_Base::Setup_triggers();
		flash_controller->ConnectToTransactionServicedSignal(handle_transaction_serviced_signal_from_PHY);
	}

	void Data_Cache_Manager_Flash_Catcher::Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats)
	{
	...
	void Data_Cache_Manager_Flash_Catcher::process_new_user_request(User_Request* user_request)
	{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					while (it != user_request->Transaction_list.end()) {
						...
						if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA)) {
							...
							} else if (available_sectors_bitmap != 0) {  /* 事务部分的sectors(available_sectors_bitmap)可以得到服务 */
					...
					if (user_request->Sectors_serviced_from_cache > 0) {  /* 如果部分事务可以在DRAM中完成 */ 
					...
					if (user_request->Transaction_list.size() > 0) {  /* 剩下的全部都是没fa在DRAM中得到完全服务的事务 */ 
						static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->Translate_lpa_to_ppa_and_dispatch(user_request->Transaction_list);
					}
	...
	void Data_Cache_Manager_Flash_Catcher::write_to_destage_buffer(User_Request* user_request)
	{
		...
		unsigned int Size_in_bytes_bypass = 0;  /* zy1024cs  统计需要bypass的大小 */
		...
		while (it != user_request->Transaction_list.end() 
			...
			if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA)) {
				...
				* This situation may result from out-of-order transaction execution*/
				...
				/* zy1024cs方案 将近期没有被访问的事务直接bypass */
				if (bloom_filter[tr->Stream_id].find(tr->LPA) == bloom_filter[tr->Stream_id].end()) {
					...
					Size_in_bytes_bypass += count_sector_no_from_status_bitmap(tr->write_sectors_bitmap);
			...
			user_request->Transaction_list.erase(it++);   /* 当back_pressure_buffer_max_depth到达一定量时 就无法删除了 */
		}
		...
		if (dram_write_size_in_sectors) {
			...
			service_dram_access_request(write_transfer_info, Size_in_bytes_bypass);  /* 建立缓存事件时需要考虑bypass的数据量的影响 */
		}
	...
	void Data_Cache_Manager_Flash_Catcher::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		*  An update read transaction is issued in Address Mapping Unit, but is consumed in data cache manager.*/
		if (transaction->Type == Transaction_Type::READ) {
			...
			switch (Data_Cache_Manager_Flash_Catcher::caching_mode_per_input_stream[transaction->Stream_id])
			{
				case Caching_Mode::TURNED_OFF:
				case Caching_Mode::WRITE_CACHE:
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{/* 可以暂时不管下面的逻辑代码 */
					if (((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, transaction->LPA)) {
						/*MQSim should get rid of writting stale data to the cache.
						* This situation may result from out-of-order transaction execution*/
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, transaction->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						...
						((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Update_data(transaction->Stream_id, transaction->LPA, content,
							timestamp, ((NVM_Transaction_Flash_RD*)transaction)->read_sectors_bitmap | slot.State_bitmap_of_existing_sectors);
					} else {
						if (!((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Check_free_slot_availability()) {
							std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
							Data_Cache_Slot_Type evicted_slot = ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_slot_lru();  /* 可以修改替换策略 Evict_one_slot_lru / Evict_one_dirty_slot */
							if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
								Memory_Transfer_Info* transfer_info = new Memory_Transfer_Info;
								...
								if (((Data_Cache_Manager_Flash_Catcher*)_my_instance)->shared_dram_request_queue) {
									sharing_id = 0;
								}
								((Data_Cache_Manager_Flash_Catcher*)_my_instance)->back_pressure_buffer_depth[sharing_id] += cache_eviction_read_size_in_sectors;
								((Data_Cache_Manager_Flash_Catcher*)_my_instance)->service_dram_access_request(transfer_info);
							}
						}
						((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Insert_read_data(transaction->Stream_id, transaction->LPA,
			...
			switch (Data_Cache_Manager_Flash_Catcher::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					((Data_Cache_Manager_Flash_Catcher*)_my_instance)->back_pressure_buffer_depth[sharing_id] -= transaction->Data_and_metadata_size_in_byte / SECTOR_SIZE_IN_BYTE + (transaction->Data_and_metadata_size_in_byte % SECTOR_SIZE_IN_BYTE == 0 ? 0 : 1);

					if (((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA)) {
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						if (((NVM_Transaction_Flash_WR*)transaction)->DataTimeStamp >= timestamp) {  /* 貌似trace一时半会不会执行 */
							((Data_Cache_Manager_Flash_Catcher*)_my_instance)->per_stream_cache[transaction->Stream_id]->Remove_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						}
					}
					
					auto user_request = ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].begin();
					while (user_request != ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].end())
					{
						((Data_Cache_Manager_Flash_Catcher*)_my_instance)->write_to_destage_buffer(*user_request);
						if ((*user_request)->Transaction_list.size() == 0) {
							((Data_Cache_Manager_Flash_Catcher*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].erase(user_request++);
						} else {
							user_request++;
						}
						//The traffic load on the backend is high and the waiting requests cannot be serviced
						if (((Data_Cache_Manager_Flash_Catcher*)_my_instance)->back_pressure_buffer_depth[sharing_id] > ((Data_Cache_Manager_Flash_Catcher*)_my_instance)->back_pressure_buffer_max_depth) {
							break;
						}
					}
	...
	void Data_Cache_Manager_Flash_Catcher::service_dram_access_request(Memory_Transfer_Info* request_info)
	{
	...
	void Data_Cache_Manager_Flash_Catcher::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		switch (eventType)
		{
			...
			case Data_Cache_Simulation_Event_Type::MEMORY_WRITE_FOR_USERIO_FINISHED:
				...
				if (is_user_request_finished((User_Request*)(transfer_info)->Related_request))  /* return (user_request->Sectors_serviced_from_cache == 0 && user_request->Transaction_list.size() == 0); */
	...
	/* zy1024cs 首次访问的事务直接bypass */
	void Data_Cache_Manager_Flash_Catcher::service_dram_access_request(Memory_Transfer_Info* request_info, unsigned int Size_in_bytes_bypass) {
		if (memory_channel_is_busy) {
			if(shared_dram_request_queue) {
				dram_execution_queue[0].push(request_info);
			} else {
			...
			Simulator->Register_sim_event(Simulator->Time() + estimate_dram_access_time(request_info->Size_in_bytes - Size_in_bytes_bypass, dram_row_size,
				dram_busrt_size, dram_burst_transfer_time_ddr, dram_tRCD, dram_tCL, dram_tRP),
				this, request_info, static_cast<int>(request_info->next_event_type));
			memory_channel_is_busy = true;
			dram_execution_list_turn = request_info->Stream_id;
