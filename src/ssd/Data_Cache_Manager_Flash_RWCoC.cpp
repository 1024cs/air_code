...
#include "Data_Cache_Manager_Flash_RWCoC.h"
...
{
	Data_Cache_Manager_Flash_RWCoC::Data_Cache_Manager_Flash_RWCoC(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
	...
	Data_Cache_Manager_Flash_RWCoC::~Data_Cache_Manager_Flash_RWCoC()
	{
		switch (sharing_mode)
		{
			case SSD_Components::Cache_Sharing_Mode::SHARED:
	...
	void Data_Cache_Manager_Flash_RWCoC::Setup_triggers()
	{
		Data_Cache_Manager_Base::Setup_triggers();
		flash_controller->ConnectToTransactionServicedSignal(handle_transaction_serviced_signal_from_PHY);
	}

	void Data_Cache_Manager_Flash_RWCoC::Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats)
	{
	...
	void Data_Cache_Manager_Flash_RWCoC::process_new_user_request(User_Request* user_request)
	{
		...
		if (Simulator->Time() > current_time_point) {
			...
			// qq("read_hit_ratio:"<<read_hit_ratio<<"  "<<"write_hit_ratio:"<<write_hit_ratio<<"  "<<user_request->band_rar[3]);
			// if (read_hit_ratio != 0.0)
			// 	qq("read_hit_ratio:"<<read_hit_ratio<<"  "<<"write_hit_ratio:"<<write_hit_ratio);
			// qq(write_hit_ratio);

			// /* 注意这里修改了缓存的模型 根据读写带宽和局部性情况调整缓存的模型 */
			// if (user_request->band_rar[0] < (user_request->band_rar[2] / 10) && user_request->band_rar[1] > 0.3 && user_request->band_rar[3] < 0.1) {
		...
		if (user_request->Type == UserRequestType::READ) {
			...
			if (user_request->band_rar[0] < (user_request->band_rar[2] / 10) && user_request->band_rar[1] > 0.2 && user_request->band_rar[3] < 0.1) {
				user_request->ifcache = true;
				// qq("R_bandwidth:"<<user_request->band_rar[0]<<"W_bandwidth:"<<user_request->band_rar[2]<<"R_RAR:"<<user_request->band_rar[1]<<"W_RAR:"<<user_request->band_rar[3])
			}
	...
	void Data_Cache_Manager_Flash_RWCoC::write_to_destage_buffer(User_Request* user_request)
	{
		...
		while (it != user_request->Transaction_list.end() 
			...
			if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA)) {  /* 缓存命中 */
				...
				* This situation may result from out-of-order transaction execution*/
				cache_transaction_write_hit_num++;
			...
			// /* 对于写请求决定是否刷回 */
			// if (user_request->band_rar[2] > 50000 && user_request->band_rar[3] < 0.01) {
		...
		if (Simulator->Time() > next_bloom_filter_reset_milestone) {
			// if (back_pressure_buffer_depth[queue_id] > 1000)
			// 	qq(back_pressure_buffer_depth[queue_id] << " " << (user_request->band_rar[2]) << " " << (user_request->band_rar[3]));
	...
	void Data_Cache_Manager_Flash_RWCoC::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		*  An update read transaction is issued in Address Mapping Unit, but is consumed in data cache manager.*/
		if (transaction->Type == Transaction_Type::READ) {
			...
			switch (Data_Cache_Manager_Flash_RWCoC::caching_mode_per_input_stream[transaction->Stream_id])
			{
				case Caching_Mode::TURNED_OFF:
				case Caching_Mode::WRITE_CACHE:
					if (transaction->UserIORequest->ifcache == true) {
						goto Read_request_to_Cache;
					}
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{/* 可以暂时不管下面的逻辑代码 */
					Read_request_to_Cache:
					if (((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, transaction->LPA)) {
						/*MQSim should get rid of writting stale data to the cache.
						* This situation may result from out-of-order transaction execution*/
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, transaction->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						...
						((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Update_data(transaction->Stream_id, transaction->LPA, content,
							timestamp, ((NVM_Transaction_Flash_RD*)transaction)->read_sectors_bitmap | slot.State_bitmap_of_existing_sectors);
					} else {
						if (!((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Check_free_slot_availability()) {
							std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
							Data_Cache_Slot_Type evicted_slot = ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_slot_lru();  /* 可以修改替换策略 Evict_one_slot_lru / Evict_one_dirty_slot */
							if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
								...
								if (((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->shared_dram_request_queue) {
									sharing_id = 0;
								}
								((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->back_pressure_buffer_depth[sharing_id] += cache_eviction_read_size_in_sectors;
								((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->service_dram_access_request(transfer_info);
							}
						}
						((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Insert_read_data(transaction->Stream_id, transaction->LPA,
			...
			switch (Data_Cache_Manager_Flash_RWCoC::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->back_pressure_buffer_depth[sharing_id] -= transaction->Data_and_metadata_size_in_byte / SECTOR_SIZE_IN_BYTE + (transaction->Data_and_metadata_size_in_byte % SECTOR_SIZE_IN_BYTE == 0 ? 0 : 1);

					if (((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA)) {
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						if (((NVM_Transaction_Flash_WR*)transaction)->DataTimeStamp >= timestamp) {  /* 貌似trace一时半会不会执行 */
							((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Remove_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						}
					}
					
					/* 主要对在waiting_user_requests_queue_for_dram_free_slot内的事务进行执行 因为本函数是从PHY来 意味着有事务已经得到了服务 释放了缓存阻塞资源 */
					auto user_request = ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].begin();
					while (user_request != ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].end())
					{
						((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->write_to_destage_buffer(*user_request);
						if ((*user_request)->Transaction_list.size() == 0) {
							((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].erase(user_request++);
						} else {
							user_request++;
						}
						//The traffic load on the backend is high and the waiting requests cannot be serviced
						if (((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->back_pressure_buffer_depth[sharing_id] > ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->back_pressure_buffer_max_depth) {
							break;
						}
					}
					...
					{
						std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
						while (!((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Empty())
						{
							DataCacheSlotType evicted_slot = ((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_dirty_slot();
						...
						if (cache_eviction_read_size_in_sectors > 0)
						{
							...
							((Data_Cache_Manager_Flash_RWCoC*)_my_instance)->service_dram_access_request(read_transfer_info);
						}
					}*/
	...
	void Data_Cache_Manager_Flash_RWCoC::service_dram_access_request(Memory_Transfer_Info* request_info)
	{
	...
	void Data_Cache_Manager_Flash_RWCoC::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
