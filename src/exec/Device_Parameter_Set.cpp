	...
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			if (strcmp(param->name(), "Seed") == 0) {
				...
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper); /* 注意这里将所有字母转换为大写 */
				...
				} else if (strcmp(val.c_str(), "JUSTITIA") == 0) {
					Caching_Mechanism = SSD_Components::Caching_Mechanism::Justitia;
				} else if (strcmp(val.c_str(), "CATCHER") == 0) {
					Caching_Mechanism = SSD_Components::Caching_Mechanism::Catcher;
				} else if (strcmp(val.c_str(), "FUZZY") == 0) {
					Caching_Mechanism = SSD_Components::Caching_Mechanism::Fuzzy;
				} else if (strcmp(val.c_str(), "RWCOC") == 0) {
					Caching_Mechanism = SSD_Components::Caching_Mechanism::RWCoC;
				} else {
				...
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);   /* 会将字母转换为大写 */
				...
				else if (strcmp(val.c_str(), "TSU_ZY") == 0)
				{
					Transaction_Scheduling_Policy = SSD_Components::Flash_Scheduling_Type::TSU_ZY;
				} else {
