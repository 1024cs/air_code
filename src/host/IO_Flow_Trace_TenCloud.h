#ifndef IO_FLOW_TRACE_TENCLOUD_H
#define IO_FLOW_TRACE_TENCLOUD_H
...
{
class IO_Flow_Trace_TenCloud : public IO_Flow_Base
{
public:
	IO_Flow_Trace_TenCloud(const sim_object_id_type &name, uint16_t flow_id, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device, uint16_t io_queue_id,
	...
	~IO_Flow_Trace_TenCloud();
	Host_IO_Request *Generate_next_request();
	void NVMe_consume_io_request(Completion_Queue_Entry *);  /* 完成一个请求 进行相关统计工作 */
	void SATA_consume_io_request(Host_IO_Request *);
	void Start_simulation();  /* 只执行一次 */
...
#endif // !IO_FLOW_TRACE_TENCLOUD_H
