	...
	public:
		...
		void Consume_pcie_message(PCIe_Message* messages)//Modern processors support DDIO, where all writes to memory are going through LLC
		{
			...
			case PCIe_Message_Type::READ_REQ:
				Read_from_memory(messages->Address, (unsigned int)(intptr_t)messages->Payload);  /* 生成Submission_Queue_Entry */
				break;
			case PCIe_Message_Type::WRITE_REQ:
				Write_to_memory(messages->Address, messages->Payload);  /* 直接进行相关数据的统计 完成此次请求 */
				break;
			default:
				break;
			}
