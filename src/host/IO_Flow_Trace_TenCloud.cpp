#include "IO_Flow_Trace_TenCloud.h"
...
{
IO_Flow_Trace_TenCloud::IO_Flow_Trace_TenCloud(const sim_object_id_type &name, uint16_t flow_id, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device, uint16_t io_queue_id,
...
IO_Flow_Trace_TenCloud::~IO_Flow_Trace_TenCloud()
{
}

Host_IO_Request *IO_Flow_Trace_TenCloud::Generate_next_request()
{
	...
	if (current_trace_line[ASCIITraceTypeColumn_TenCloud].compare(ASCIITraceWriteCode_TenCloud) == 0)
	{
	...
	char *pEnd;
	request->LBA_count = std::strtoul(current_trace_line[ASCIITraceSizeColumn_TenCloud].c_str(), &pEnd, 0);

	request->Start_LBA = std::strtoull(current_trace_line[ASCIITraceAddressColumn_TenCloud].c_str(), &pEnd, 0);

	// qq(current_trace_line[0]<<" "<<request->Start_LBA<<" "<<request->LBA_count<<" "<<current_trace_line[ASCIITraceTypeColumn_TenCloud]);
...
void IO_Flow_Trace_TenCloud::NVMe_consume_io_request(Completion_Queue_Entry *io_request)
{
	IO_Flow_Base::NVMe_consume_io_request(io_request);  /* 完成一个请求 进行相关统计工作 */
	IO_Flow_Base::NVMe_update_and_submit_completion_queue_tail();
}

void IO_Flow_Trace_TenCloud::SATA_consume_io_request(Host_IO_Request *io_request)
{
	IO_Flow_Base::SATA_consume_io_request(io_request);
}

void IO_Flow_Trace_TenCloud::Start_simulation()
{
	...
	while (std::getline(trace_file, trace_line))
	{
		...
		Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter_TenCloud, current_trace_line);
		...
		if (current_trace_line.size() != ASCIIItemsPerLine_TenCloud)
		{
			break;
		}
		total_requests_in_file++;
		sim_time_type prev_time = last_request_arrival_time;
		last_request_arrival_time = std::strtoll(current_trace_line[ASCIITraceTimeColumn_TenCloud].c_str(), &pEnd, 10);
	...
	curFireTime = std::strtoll(current_trace_line[ASCIITraceTimeColumn_TenCloud].c_str(), &pEnd, 10);
...
void IO_Flow_Trace_TenCloud::Validate_simulation_config()
{
}

void IO_Flow_Trace_TenCloud::Execute_simulator_event(MQSimEngine::Sim_Event *)
{
	...
	if (STAT_generated_request_count < total_requests_to_be_generated)
	{
		...
		if (std::getline(trace_file, trace_line))   /* 赋值一行给trace_line */
		{
			...
			current_trace_line.clear();
			Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter_TenCloud, current_trace_line);    /* 每个元素丢到新的current_trace_line，和上面不一样 */
			// current_trace_line[3] = current_trace_line[3].substr(0, 1);  /* 去掉TenCloud中读写后面的, */
		}
		else
		{   /* 一般不会执行下面 */
...
void IO_Flow_Trace_TenCloud::Get_statistics(Utils::Workload_Statistics &stats, LPA_type (*Convert_host_logical_address_to_device_address)(LHA_type lha),
...
{   /* 执行在host_System，但是需要preconditioning_required */
	...
	while (std::getline(trace_file_temp, trace_line))
	{
		Utils::Helper_Functions::Remove_cr(trace_line);
		line_splitted.clear();
		Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter_TenCloud, line_splitted);
		// current_trace_line[3] = current_trace_line[3].substr(0, 1);  /* 去掉TenCloud中读写后面的, */
		if (line_splitted.size() != ASCIIItemsPerLine_TenCloud)
		{
			break;
		}
		sim_time_type prev_time = last_request_arrival_time;
		last_request_arrival_time = std::strtoull(line_splitted[ASCIITraceTimeColumn_TenCloud].c_str(), &pEnd, 10);
		if (last_request_arrival_time < prev_time)
		{
		...
		unsigned int LBA_count = std::strtoul(line_splitted[ASCIITraceSizeColumn_TenCloud].c_str(), &pEnd, 0);
		sum_request_size += LBA_count;
		LHA_type start_LBA = std::strtoull(line_splitted[ASCIITraceAddressColumn_TenCloud].c_str(), &pEnd, 0);
		...
		while (start_LBA <= end_LBA)
		{
			...
			if (line_splitted[ASCIITraceTypeColumn_TenCloud].compare(ASCIITraceWriteCode_TenCloud) == 0)
			{
