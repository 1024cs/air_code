...
namespace SSD_Components
{
	...
	Cached_Mapping_Table::~Cached_Mapping_Table()
	{
		...
		/* zy1024cs改 前面的指针已经删除 后面不需要了 */
		while (!lruList.empty()) {
			// auto entry = lruList.front();
			// delete entry.second;
			lruList.pop_front();
		}
	}
	...
	{   /* 判断内部是否存在info似乎与下面的Insert_new_mapping_info逻辑是相反的 该函数首先保证存在key并且VALID 然后对ppa更新 并设置为脏mapping */
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		assert(it != addressMap.end());
		assert(it->second->Status == CMTEntryStatus::VALID);
	...
	{	/* 前面一些函数保证了内部addressMap存在记录 然后设置为VALID 更新ppa 最后设置为非脏mapping */
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		if (it == addressMap.end()) {
	...
	{	/* 在CMT中插入了一个新的lpn 但是状态设置为WAITING 配合Insert_new_mapping_info使用 其中会设置为VALID */
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		...
		PlaneAllocationScheme(PlaneAllocationScheme), Channel_no(channel_no), Chip_no(chip_no), Die_no(die_no), Plane_no(plane_no)
	{
		...
		Channel_ids = new flash_channel_ID_type[channel_no];  /* channel_no是按照flow_channel_id_assignments内部数量决定 */
		for (flash_channel_ID_type cid = 0; cid < channel_no; cid++) {
			Channel_ids[cid] = channel_ids[cid];  /* channel_ids即为exec/SSD_Device.cpp中的flow_channel_id_assignments 但是是在Address_Mapping_Unit_Page_Level::Address_Mapping_Unit_Page_Level中传过来的 */
		}
		...
		if (CMT == NULL) {	/* 如果在Address_Mapping_Unit_Page_Level::Address_Mapping_Unit_Page_Level中被判定为CMT_Sharing_Mode::SHARED才会被提前赋值传进来 */
			//Each flow (address mapping domain) has its own CMT, so CMT is create here in the constructor
			this->CMT = new Cached_Mapping_Table(cmt_capacity);  /* 此时的cmt_capacity == per_stream_cmt_capacity = cmt_capacity / no_of_input_streams; */
		} else {
			//The entire CMT space is shared among concurrently running flows (i.e., address mapping domains of all flow)
			this->CMT = CMT;
		}

		/* 对所有的逻辑translation在全局Translation的字典中进行注册 */
	...
	{	/* 提前判断有没有 是其他譬如Cached_Mapping_Table::Update_mapping_info和Get_page_status的前驱 */
		if (ideal_mapping) {
			return true;
		} else {
			...
			Block_no_per_plane, Page_no_per_block, SectorsPerPage, PageSizeInByte, Overprovisioning_ratio, sharing_mode, fold_large_addresses)
	{
		...
		/* 基本上没什么用 就处理一些空间不足导致的事务 不会进一步处理 */
		...
		for (unsigned int domainID = 0; domainID < no_of_input_streams; domainID++) {
			...
			*/
			/* 计算每个CMT_entry 每个GTD_entry 的大小 计算每个page中可以放几个translation_entry */
			...
			cmt_capacity = cmt_capacity_in_byte / CMT_entry_size;  /* 主要用来确定缓存中CMT可以放几个 */
			switch (sharing_mode) {
			...
			for (unsigned int i = 0; i < stream_channel_ids[domainID].size(); i++) {
				if (stream_channel_ids[domainID][i] < channel_count) {
					channel_ids[i] = stream_channel_ids[domainID][i];  /* stream_channel_ids即为exec/SSD_Device.cpp中的flow_channel_id_assignments */
				} else {
	...
	{
		if (mapping_table_stored_on_flash) {  /* 在src/ssd/Address_Mapping_Unit_Base.cpp中被mapping_table_stored_on_flash(false) */
			return;
		}
		...
		for (unsigned int stream_id = 0; stream_id < no_of_input_streams; stream_id++) {
			...
			for (MVPN_type translation_page_id = 0; translation_page_id < domains[stream_id]->Total_translation_pages_no; translation_page_id++) {
				dummy_tr->LPA = (LPA_type)translation_page_id;  /* 对应于mvpn */
				allocate_plane_for_translation_write(dummy_tr);  /* 按照Flash_Plane_Allocation_Scheme_Type分配dummy_tr的物理地址Address */
				allocate_page_in_plane_for_translation_write(dummy_tr, (MVPN_type)dummy_tr->LPA, false);
				flash_controller->Change_flash_page_status_for_preconditioning(dummy_tr->Address, dummy_tr->LPA);	/* 就是在src/nvm_chip/flash_memory/Flash_Chip.cpp中将Pages.Metadata.LPA = *(LPA_type*)status_info; */
			}
		}
	...
	void Address_Mapping_Unit_Page_Level::Translate_lpa_to_ppa_and_dispatch(const std::list<NVM_Transaction*>& transactionList)
	{
		...
		for (std::list<NVM_Transaction*>::const_iterator it = transactionList.begin(); it != transactionList.end(); it++){
			(*it)->mapping_time = Simulator->Time();
		}
		...
		for (std::list<NVM_Transaction*>::const_iterator it = transactionList.begin(); it != transactionList.end(); ) {
			if (is_lpa_locked_for_gc((*it)->Stream_id, ((NVM_Transaction_Flash*)(*it))->LPA)) {
				...
				manage_user_transaction_facing_barrier((NVM_Transaction_Flash*)*(it++));  /* 插入至Read/Write_transactions_behind_LPA_barrier */
			} else {
				query_cmt((NVM_Transaction_Flash*)(*it++));  /* 主要对事务进行Physical_address_determined确定 然后统一进行调度 */
			}
		}
				...
				if (((NVM_Transaction_Flash*)(*it))->Physical_address_determined) {
					...
					if (((NVM_Transaction_Flash*)(*it))->Type == Transaction_Type::WRITE) {
						if (((NVM_Transaction_Flash_WR*)(*it))->RelatedRead != NULL) {
							ftl->TSU->Submit_transaction(((NVM_Transaction_Flash_WR*)(*it))->RelatedRead);  /* 对于写请求还要将关联读也丢进去 */
						}
					}
				}
			}
	...
	bool Address_Mapping_Unit_Page_Level::query_cmt(NVM_Transaction_Flash* transaction)
	{
		...
		if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, transaction->LPA))//Either limited or unlimited CMT
		{
			...
			if (translate_lpa_to_ppa(stream_id, transaction)) {
				...
				mange_unsuccessful_translation(transaction);  /* 只能可能是空间不足导致 直接添加到Write_transactions_for_overfull_planes 之后不处理 */
				return false;
			}
		} else {//Limited CMT
			//Maybe we can catch mapping data from an on-the-fly write back request
			if (request_mapping_entry(stream_id, transaction->LPA)) {  /* 如果执行成功则表明已经放入CMT中了 */
				Stats::CMT_miss++;
	...
	*/
	bool Address_Mapping_Unit_Page_Level::translate_lpa_to_ppa(stream_id_type streamID, NVM_Transaction_Flash* transaction)
	{
		...
		if (transaction->Type == Transaction_Type::READ) {
			if (ppa == NO_PPA) {  /* 感觉可能是对于某些地址还没写的情况下就被先读的时候会发生 */
			...
			allocate_plane_for_user_write((NVM_Transaction_Flash_WR*)transaction);  /* 分配channel到plane的位置 */
			//there are too few free pages remaining only for GC
			if (ftl->GC_and_WL_Unit->Stop_servicing_writes(transaction->Address)){
				return false;
			}
			allocate_page_in_plane_for_user_write((NVM_Transaction_Flash_WR*)transaction, false);  /* 分配block和page的位置 */
			transaction->Physical_address_determined = true;
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc)
	{
		...
		if (old_ppa == NO_PPA)  /* this is the first access to the logical page 第一次写请求来的时候原先是没有old_ppa的 */
		{
			if (is_for_gc) {
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_translation_write(NVM_Transaction_Flash* transaction, MVPN_type mvpn, bool is_for_gc)
	{
		...
		if (old_MPPN == NO_MPPN) {
			...
			block_manager->Invalidate_page_in_block(transaction->Stream_id, prevAddr);  /* 先无效之前的Invalid_page_bitmap地址位 */
		}

		block_manager->Allocate_block_and_page_in_plane_for_translation_write(transaction->Stream_id, transaction->Address, false);  /* 在plane_record->Translation_wf中分配 */
		transaction->PPA = Convert_address_to_ppa(transaction->Address);
	...
	{
		AddressMappingDomain* domain = domains[stream_id];
		MVPN_type mvpn = get_MVPN(lpa, stream_id);  /* return (MVPN_type)(lpn / no_of_translation_entries_per_page); */
			...
			if (!domain->CMT->Check_free_slot_availability()) {
				...
				if (evictedItem.Dirty) {
					...
					* after eviction happens.*/
					...
					generate_flash_writeback_request_for_mapping_data(stream_id, evicted_lpa);  /* Read the unchaged mapping entries from flash to merge them with updated parts of MVPN */
				}
			}
			domain->CMT->Reserve_slot_for_lpn(stream_id, lpa);  /* 添加到lruList和addressMap */
		...
		* 2. A read has been issued to retrieve the mapping data for some previous user requests */
		...
		{
			if (domain->CMT->Is_slot_reserved_for_lpn_and_waiting(stream_id, lpa)) {  /* if (it->second->Status == CMTEntryStatus::WAITING) */
				return false;
	...
	inline void Address_Mapping_Unit_Page_Level::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		} else {	/* 貌似在Waiting_unmapped_read_transactions和Waiting_unmapped_program_transactions中寻找与当前读相同的lpa来一起事务执行 */
