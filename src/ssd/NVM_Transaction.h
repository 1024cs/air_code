...
namespace SSD_Components
{
	...
	public:
		...
		sim_time_type Issue_time;   /* 事务被创建时的时间 在析构函数中直接赋值为Simulator->Time() */
		...
		sim_time_type STAT_execution_time, STAT_transfer_time;  /* 均在NVM_PHY_ONFI_NVDDR2.cpp中被计算  在测试ID_322和ID_355的时候发现STAT_execution_time始终为75020*/

		/* zy1024cs */
		long int cache_time = -1;
		long int mapping_time = -1;
		long int tsu_time = -1;
		long int flash_time = -1;

		std::string Info_of_turn(){
			std::string obj;
			if (cache_time != -1){
				if (mapping_time != -1){
					if (tsu_time != -1){
						if (flash_time != -1){
							obj = "Cache_turn: " + std::to_string(mapping_time - cache_time) + " Mapping_turn: " + std::to_string(tsu_time - mapping_time) + " Tsu_turn: " + std::to_string(flash_time - tsu_time);
						} else {
							obj = "Cache_turn: " + std::to_string(mapping_time - cache_time) + " Mapping_turn: " + std::to_string(tsu_time - mapping_time) + " Tsu_turn: " + std::to_string(-1);
						}
					} else {
						obj = "Cache_turn: " + std::to_string(mapping_time - cache_time) + " Mapping_turn: " + std::to_string(-1) + " Tsu_turn: " + std::to_string(-1);
					}
				} else {
					obj = "Cache_turn: " + std::to_string(-1) + " Mapping_turn: " + std::to_string(-1) + " Tsu_turn: " + std::to_string(-1);
				}
			} else {
				obj = "Cache_turn: " + std::to_string(-1) + " Mapping_turn: " + std::to_string(-1) + " Tsu_turn: " + std::to_string(-1);
			}

			// if (mapping_time - cache_time != 0)
			// 	qq(obj);
			return obj;
		}
