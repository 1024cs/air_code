...
private:
    Flash_Transaction_Queue ***UserReadTRQueue;  /* UserReadTRQueue[channelID][chipId][priorityClass] */
    ...
    IO_Flow_Priority_Class::Priority **nextPriorityClassRead;  /* 最后在UserReadTRQueue的优先级地方发挥作用 nextPriorityClassRead[channelID][chipId] */
    IO_Flow_Priority_Class::Priority **nextPriorityClassWrite;
    int **currentWeightRead;  /* currentWeightRead[channelID][chipId] */
    ...
    Flash_Transaction_Queue *get_next_read_service_queue(NVM::FlashMemory::Flash_Chip *chip);  /* 凡是涉及到UserReadTRQueue赋值的地方就会被调用 内部会根据Priority进行调用 */
    Flash_Transaction_Queue *get_next_write_service_queue(NVM::FlashMemory::Flash_Chip *chip);  /* 凡是涉及到UserWriteTRQueue赋值的地方就会被调用 内部会根据Priority进行调用 */
};
} // namespace SSD_Components
