...
namespace SSD_Components
{
	...
	{
	public:
		ChipStatus Status;   /* { IDLE, CMD_IN, CMD_DATA_IN, DATA_OUT, READING, WRITING, ERASING, WAIT_FOR_DATA_OUT, WAIT_FOR_COPYBACK_CMD } */
	...
	public:
		...
		void Setup_triggers();  /* 内部实现将自身的handle_ready_signal_from_chip加入到chip层面完成的广播 */
		...
		bool HasSuspendedCommand(NVM::FlashMemory::Flash_Chip* chip);  /* return bookKeepingTable[chip->ChannelID][chip->ChipID].HasSuspend */
		ChipStatus GetChipStatus(NVM::FlashMemory::Flash_Chip* chip);
		sim_time_type Expected_finish_time(NVM::FlashMemory::Flash_Chip* chip);  /* 针对具体的chip */
		sim_time_type Expected_finish_time(NVM_Transaction_Flash* transaction);  /* 针对当前事务的执行chip */
		sim_time_type Expected_transfer_time(NVM_Transaction_Flash* transaction);
		NVM_Transaction_Flash* Is_chip_busy_with_stream(NVM_Transaction_Flash* transaction);  /* 返回对应事务相同流的正在目标chip上活跃的事务 */
	...
	private:
		...
		static void handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command);  /* 被添加chip层面实现广播 */
