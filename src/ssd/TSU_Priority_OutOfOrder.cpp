...
{  /* 按照事务的Priority_class来添加到对应的queue */
    ...
    for (std::list<NVM_Transaction_Flash *>::iterator it = transaction_receive_slots.begin();
         ...
         it++)
    {
        //std::cout<<((*it)->UserIORequest)<<std::endl;
        // qq(((int)(*it)->Priority_class));
        // std::abort();
        ...
        case Transaction_Type::READ:
            ...
            {
                // PRINT_MESSAGE((*it)->LPA);
                //PRINT_MESSAGE("read-"<<((*it)->UserIORequest->Type==UserRequestType::READ)<<"-"<<(*it)->UserIORequest->Size_in_byte);
                // std::abort();
                if ((*it)->Priority_class != IO_Flow_Priority_Class::UNDEFINED)
                {

                    // if ((*it)->UserIORequest->Size_in_byte < 16384){
                    //     (*it)->Priority_class = IO_Flow_Priority_Class::URGENT;
                    // }
                    // if ((*it)->Priority_class != IO_Flow_Priority_Class::Priority::HIGH)
                    //     PRINT_MESSAGE(IO_Flow_Priority_Class::to_string((*it)->Priority_class));

                    UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)].push_back((*it));
                    
                    // if ((*it)->UserIORequest->Size_in_byte < 16384){
                    //     UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][IO_Flow_Priority_Class::URGENT].push_back((*it));
                    // }
                    // else{
                    //     UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)].push_back((*it));
                    // }
                }
                else  /* (*it)->Priority_class == IO_Flow_Priority_Class::UNDEFINED */
                {
        ...
        case Transaction_Type::WRITE:
            ...
            case Transaction_Source_Type::CACHE:
            case Transaction_Source_Type::USERIO:
                //PRINT_MESSAGE("write-"<<((*it)->UserIORequest->Type==UserRequestType::WRITE)<<"-"<<(*it)->UserIORequest->Size_in_byte);
                if ((*it)->Priority_class != IO_Flow_Priority_Class::UNDEFINED)
                {
...
{
    if (UserReadTRQueue[chip->ChannelID][chip->ChipID][IO_Flow_Priority_Class::URGENT].size() > 0)  /* URGENT事务始终优先 */
    {
