...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	public:
		...
		PPA_type Retrieve_ppa(const stream_id_type streamID, const LPA_type lpa);  /* 吧访问的放在lru前面 */
		...
		page_status_type Get_bitmap_vector_of_written_sectors(const stream_id_type streamID, const LPA_type lpa);  /* return addressMap.find(key)->second->WrittenStateBitmap */

		bool Is_slot_reserved_for_lpn_and_waiting(const stream_id_type streamID, const LPA_type lpa);  /* 状态Status是否处于WAITING 其他情况均为false 包括lpn不存在 */
		bool Check_free_slot_availability();  /* return addressMap.size() < capacity; */
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		...
		* proposal. It is always stored in non-volatile flash memory.*/
		...
		bool Mapping_entry_accessible(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa);  /* 是否ideal_mapping或者CMT->Exists(stream_id, lpa); */
		...
		std::multimap<LPA_type, NVM_Transaction_Flash*> Read_transactions_behind_LPA_barrier;   /* zy1024cs 由is_lpa_locked_for_gc判断是否应该放进去 */
		...
		MVPN_type Total_translation_pages_no;  /* Total_translation_pages_no = MVPN_type(Total_logical_pages_no / Translation_entries_per_page); */
	};
	...
	public:
		...
		void Setup_triggers();
		void Start_simulation();  /* Store_mapping_table_on_flash_at_start(); */
		void Validate_simulation_config();  /* 什么都没做 */
		void Execute_simulator_event(MQSimEngine::Sim_Event*);  /* 什么都没做 */
		...
		unsigned int Get_current_cmt_occupancy_for_stream(stream_id_type stream_id);  /* return domains[stream_id]->No_of_inserted_entries_in_preconditioning; */
	...
	private:
		...
		AddressMappingDomain** domains;  /* new AddressMappingDomain*[no_of_input_streams]; */
		...
		std::set<NVM_Transaction_Flash_WR*>**** Write_transactions_for_overfull_planes;  /* 用来存放空间不足时候的事务 但是基本不会进一步处理 */
		...
		MVPN_type get_MVPN(const LPA_type lpn, stream_id_type stream_id);  /* return (MVPN_type)(lpn / no_of_translation_entries_per_page); */
