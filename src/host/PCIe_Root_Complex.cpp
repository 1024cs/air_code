...
namespace Host_Components
{
	...
	void PCIe_Root_Complex::Write_to_memory(const uint64_t address, const void* payload)  /* host端的内存 不是盘设备内部 */
	{
		...
		if (address >= DATA_MEMORY_REGION) {
			...
			switch (SSD_device_type) {
				case HostInterface_Types::NVME:
				{
					...
					((*IO_flows)[flow_id])->NVMe_consume_io_request((Completion_Queue_Entry*)payload);   /* 直接进行相关数据的统计 完成此次请求 */
					break;
				}
	...
	void PCIe_Root_Complex::Write_to_device(uint64_t address, uint16_t write_value)  /* 一个示例Write_to_device(nvme_queue_pair.Submission_tail_register_address_on_device, nvme_queue_pair.Submission_queue_tail) */
	{
		...
		if (address >= DATA_MEMORY_REGION) {
			...
			switch (SSD_device_type) {
				case HostInterface_Types::NVME:
				{
					...
					new_pcie_message->Payload = (*IO_flows)[flow_id]->NVMe_read_sqe(address);   /* 生成Submission_Queue_Entry */
