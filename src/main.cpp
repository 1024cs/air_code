...
void command_line_args(char* argv[], string& input_file_path, string& workload_file_path)
{
	...
	// 		qq(workload_file_path);
	// 		continue;
	// 	}
	// }
	input_file_path = "ssdconfig.xml";
	workload_file_path = "workload.xml";
}
...
string Get_file_name(string obj) {
	int start = obj.find_last_of("/"), end = obj.find_last_of(".");
...
int main(int argc, char* argv[])
{
	...
	std::vector<std::vector<string>> log;
	std::vector<std::vector<int>> log1;
	std::vector<std::vector<string>> log2;
	std::vector<std::vector<string>> log3;
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
		...
		for (auto io_flow_def = (*io_scen)->begin(); io_flow_def != (*io_scen)->end(); io_flow_def++) {  /* 对IO_Scenario内的多个IO负载进行添加 */
		...
		collect_results(ssd, host, ("results/" + workload_defs_file_path.substr(0, workload_defs_file_path.find_last_of(".")) + "_scenario_" + std::to_string(cntr) + ".xml").c_str());


		/* zy1024cs 用于最后结果收集 */
		std::vector<string> obj;
		std::vector<int> obj1;
		std::vector<string> obj2;
		std::vector<string> obj3;
		std::vector<Host_Components::IO_Flow_Base*> IO_flows = host.Get_io_flows();
		for (unsigned int stream_id = 0; stream_id < IO_flows.size(); stream_id++) {

			obj.push_back(Get_file_name(((IO_Flow_Parameter_Set_Trace_Based*)exec_params->Host_Configuration.IO_Flow_Definitions[stream_id])->File_Path));
			obj1.push_back(IO_flows[stream_id]->Get_end_to_end_request_delay());
			obj2.push_back((ssd.Host_interface)->input_stream_manager->Get_transaction_statistics_string(stream_id));
			obj3.push_back((ssd.Cache_manager)->Get_hit_ratio(stream_id));
		}

		obj3.push_back((ssd.Cache_manager)->Get_hit_ratio());
	...
	/* zy1024cs 完整打印并收集测试结果 */

	for (int i = 0; i < log1.size(); i++) {
		for (int j = 0; j < log[i].size(); j++) {
			ofs<<log[i][j]<<",";
		}
		for (int j = 0; j < log1[i].size(); j++) {
			ofs<<log1[i][j]<<",";
		}
		for (int j = 0; j < log3[i].size(); j++) {
			ofs<<log3[i][j]<<",";
		}
		ofs<<std::endl;
	}
	...
	while (log.size() > 0) {
		std::vector<string> obj = log.front();
		log.erase(log.begin());
	...
	while (log1.size() > 0) {
		std::vector<int> obj = log1.front();
		log1.erase(log1.begin());
		for (auto it = obj.begin(); it != obj.end(); it++) {
			std::cout<<(*it)<<" ";
		}
		std::cout<<std::endl;
	...
	while (log2.size() > 0) {
		std::vector<string> obj = log2.front();
		log2.erase(log2.begin());
		for (auto it = obj.begin(); it != obj.end(); it++) {
			std::cout<<(*it)<<" ";
		}
		std::cout<<std::endl;
	...
	while (log3.size() > 0) {
		std::vector<string> obj = log3.front();
		log3.erase(log3.begin());
		for (auto it = obj.begin(); it != obj.end(); it++) {
			std::cout<<(*it)<<" ";
		}
		std::cout<<std::endl;
