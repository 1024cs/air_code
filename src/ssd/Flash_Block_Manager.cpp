...
namespace SSD_Components
{
	...
	void Flash_Block_Manager::Allocate_block_and_page_in_plane_for_user_write(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		if(plane_record->Data_wf[stream_id]->Current_page_write_index == pages_no_per_block) {  /* 一个block已经写满了 */
			//Assign a new write frontier block
			plane_record->Data_wf[stream_id] = plane_record->Get_a_free_block(stream_id, false);  /* 从Free_block_pool中选择一个free块返回 */
			gc_and_wl_unit->Check_gc_required(plane_record->Get_free_block_pool_size(), page_address);  /* 检查是否需要进行GC回收 */
		}
