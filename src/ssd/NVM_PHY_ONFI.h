...
namespace SSD_Components
{
	...
	public:
		...
		virtual void Send_command_to_chip(std::list<NVM_Transaction_Flash*>& transactionList) = 0;   /* TSU调度完成后进入闪存介质的入口 */
	...
	protected:
		...
		std::vector<TransactionServicedHandlerType> connectedTransactionServicedHandlers;  /* 包含mapping GC cache 和 TSU */
		void broadcastTransactionServicedSignal(NVM_Transaction_Flash* transaction);
		std::vector<ChannelIdleHandlerType> connectedChannelIdleHandlers;  /* 只有TSU */
		void broadcastChannelIdleSignal(flash_channel_ID_type);
		std::vector<ChipIdleHandlerType> connectedChipIdleHandlers;  /* 只有TSU */
