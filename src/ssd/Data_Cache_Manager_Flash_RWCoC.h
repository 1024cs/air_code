#ifndef Data_Cache_Manager_Flash_RWCoC_H
#define Data_Cache_Manager_Flash_RWCoC_H
...
namespace SSD_Components
{
	...
	Assumed hardware structure:
			...
			|     DRAM Data_Cache_Flash Main Data Space                   |   Back Pressure Space  | ---------->To the flash backend
			 --------------------------------------------------|------------------------
	*/
	class Data_Cache_Manager_Flash_RWCoC : public Data_Cache_Manager_Base
	{
	public:
		Data_Cache_Manager_Flash_RWCoC(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
		...
		~Data_Cache_Manager_Flash_RWCoC();
	...
	private:
		...
		unsigned int back_pressure_buffer_max_depth;  /* 以plane为最小粒度的全部个数 也就是能够实现最理想状态下的plane并行的数量 */
...
#endif // !Data_Cache_Manager_Flash_RWCoC_H
