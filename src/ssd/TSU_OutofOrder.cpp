...
{   /* 按照每个channel每个chip来安排对应的user GC和mapping */
...
{	/* 将transaction_receive_slots中的每一个事务按照USERIO, CACHE, GC_WL, MAPPING几种类型放入相应的[channal][chip]数组中后轮番对每个channal内的每个chip进行process_chip_requests(先read后write再erase) */
	...
	/* 将transaction_receive_slots中的每一个事务按照USERIO, CACHE, GC_WL, MAPPING几种类型放入相应的[channal][chip]数组 */
	...
	/* 轮询每个channel每个chip中的user GC和mapping事务列表 */
	for (flash_channel_ID_type channelID = 0; channelID < channel_count; channelID++)
	{
		if (_NVMController->Get_channel_status(channelID) == BusChannelStatus::IDLE)
		{
			for (unsigned int i = 0; i < chip_no_per_channel; i++)
			{
				...
				if (_NVMController->Get_channel_status(chip->ChannelID) != BusChannelStatus::IDLE)  /* process_chip_requests起作用了 因此对应chip的channel被占用 应该轮询下一个channel */
				{
					break;   /* 遍历下一个channel */
				}
			}
...
{	/* 优先mapping 再GC 再read */
	...
	if (MappingReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
	{
		...
		if (ftl->GC_and_WL_Unit->GC_is_in_urgent_mode(chip) && GCReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)  /* GC是属于紧急模式了 */
		{
	...
	{
		//If flash transactions related to GC are prioritzed (non-preemptive execution mode of GC (preemptible_gc_enabled)), then GC queues are checked first
		...
		else if (GCWriteTRQueue[chip->ChannelID][chip->ChipID].size() > 0)  /* 跳转出函数去执行write */
		{
			return false;
		}
	...
	case ChipStatus::IDLE:
		break;  /* 当前chip可以直接执行相关事务 */
	case ChipStatus::WRITING:
		if (!programSuspensionEnabled || _NVMController->HasSuspendedCommand(chip))  /* Suspension没有开启或者当前已有Suspension事务正在执行 */
		{
			return false;
		}
		if (_NVMController->Expected_finish_time(chip) - Simulator->Time() < writeReasonableSuspensionTimeForRead)  /* 如果完成时间不长 可以没必要打断 */
		{
			return false;
		}
