...
#include "Data_Cache_Manager_Flash_Fuzzy.h"
...
{
	Data_Cache_Manager_Flash_Fuzzy::Data_Cache_Manager_Flash_Fuzzy(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
		...
		dram_execution_list_turn(0), back_pressure_buffer_max_depth(back_pressure_buffer_max_depth)
	{
		...
		/* zy1024cs */
		this->stream_bytes_read = new sim_time_type[stream_count];
		this->stream_bytes_write = new sim_time_type[stream_count];
		this->stream_fuzzy_priority = new int[stream_count];
		for (unsigned int i = 0; i < stream_count; i++) {
			this->stream_bytes_read[i] = 0;
			this->stream_bytes_write[i] = 0;
			this->stream_fuzzy_priority[i] = -1;
		}

		bloom_filter = new std::set<LPA_type>[stream_count];
	}
	
	Data_Cache_Manager_Flash_Fuzzy::~Data_Cache_Manager_Flash_Fuzzy()
	{
		switch (sharing_mode)
		{
			case SSD_Components::Cache_Sharing_Mode::SHARED:
	...
	void Data_Cache_Manager_Flash_Fuzzy::Setup_triggers()
	{
		Data_Cache_Manager_Base::Setup_triggers();
		flash_controller->ConnectToTransactionServicedSignal(handle_transaction_serviced_signal_from_PHY);
	}

	void Data_Cache_Manager_Flash_Fuzzy::Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats)
	{
	...
	void Data_Cache_Manager_Flash_Fuzzy::process_new_user_request(User_Request* user_request)
	{
		...
		if (Simulator->Time() > current_time_point) {
			...
			// // qq("read_hit_ratio:"<<read_hit_ratio<<"  "<<"write_hit_ratio:"<<write_hit_ratio);
			// cache_transaction_read_num = 0, cache_transaction_write_num = 0;
			// cache_transaction_read_hit_num = 0, cache_transaction_write_hit_num = 0;

			sim_time_type* bandwith = new sim_time_type[stream_count];
			for (unsigned int i = 0; i < stream_count; i++) {
				bandwith[i] = (stream_bytes_read[i] + stream_bytes_write[i]);  /* 理论上需要除以real_time_interval 简便起见不用 */
			}
			/* 针对Fuzzy进行用户I/O的优先级重指定 注意bandwith内部时常出现有多个0的情况 */
			std::sort(bandwith, bandwith + stream_count);
			// for (unsigned int i = 0; i < stream_count; i++) {
			// 	std::cout<<bandwith[i]<<"-";
			// }
			// std::cout<<std::endl;

			sim_time_type user_stream_intensity = stream_bytes_read[user_request->Stream_id] + stream_bytes_write[user_request->Stream_id];
			for (unsigned int i = 0; i < stream_count; i++) {
				if (bandwith[i] == user_stream_intensity) {
					user_request->fuzzy_priority = i;
					break;
				}
			}

			waiting_user_requests_queue_for_dram_free_slot->sort(Cmp);
		...
		if (user_request->Type == UserRequestType::READ) {
			stream_bytes_read[user_request->Stream_id] += user_request->Size_in_byte;
			switch (caching_mode_per_input_stream[user_request->Stream_id]) {
				case Caching_Mode::TURNED_OFF:
					...
					return;
				case Caching_Mode::WRITE_CACHE:
				case Caching_Mode::READ_CACHE:  /* 这里似乎没有对在cache中得不到服务的read事务将其内容从闪存写入cache */
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					while (it != user_request->Transaction_list.end()) {
						...
						if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA)) {  /* 缓存命中 */
							...
							if (available_sectors_bitmap == tr->read_sectors_bitmap) {
								user_request->Sectors_serviced_from_cache += count_sector_no_from_status_bitmap(tr->read_sectors_bitmap);  /* 事务所有的sectors可以得到服务 */
					...
					if (user_request->Transaction_list.size() > 0) {  /* 剩下的全部都是没fa在DRAM中得到完全服务的读事务 需要把这些事务执行玩一个用户请求才算执行完毕 */ 
						static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->Translate_lpa_to_ppa_and_dispatch(user_request->Transaction_list);
					}
		...
		{
			stream_bytes_write[user_request->Stream_id] += user_request->Size_in_byte;
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				case Caching_Mode::TURNED_OFF:
				case Caching_Mode::READ_CACHE:
	...
	void Data_Cache_Manager_Flash_Fuzzy::write_to_destage_buffer(User_Request* user_request)
	{
		...
		while (it != user_request->Transaction_list.end() 
			...
			// /* hot/cold data separation 如果开启冷热分类 下面的也要打开 (因为事务来源于用户 所以这种急切的写回属于用户事务)*/
		...
		if (writeback_transactions.size() > 0) {   /* 写回的是当前来的用户请求中的事务 因此不需要从缓存中读数据 因为无需建立单独的访问缓存事件 */
	...
	void Data_Cache_Manager_Flash_Fuzzy::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		*  An update read transaction is issued in Address Mapping Unit, but is consumed in data cache manager.*/
		if (transaction->Type == Transaction_Type::READ) {
			...
			switch (Data_Cache_Manager_Flash_Fuzzy::caching_mode_per_input_stream[transaction->Stream_id])
			{
				case Caching_Mode::TURNED_OFF:
				case Caching_Mode::WRITE_CACHE:
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{/* 可以暂时不管下面的逻辑代码 */
					if (((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, transaction->LPA)) {
						/*MQSim should get rid of writting stale data to the cache.
						* This situation may result from out-of-order transaction execution*/
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, transaction->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						...
						((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Update_data(transaction->Stream_id, transaction->LPA, content,
							timestamp, ((NVM_Transaction_Flash_RD*)transaction)->read_sectors_bitmap | slot.State_bitmap_of_existing_sectors);
					} else {
						if (!((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Check_free_slot_availability()) {
							std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
							Data_Cache_Slot_Type evicted_slot = ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_slot_lru();  /* 可以修改替换策略 Evict_one_slot_lru / Evict_one_dirty_slot */
							if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
								...
								if (((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->shared_dram_request_queue) {
									sharing_id = 0;
								}
								((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->back_pressure_buffer_depth[sharing_id] += cache_eviction_read_size_in_sectors;
								((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->service_dram_access_request(transfer_info);
							}
						}
						((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Insert_read_data(transaction->Stream_id, transaction->LPA,
			...
			switch (Data_Cache_Manager_Flash_Fuzzy::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->back_pressure_buffer_depth[sharing_id] -= transaction->Data_and_metadata_size_in_byte / SECTOR_SIZE_IN_BYTE + (transaction->Data_and_metadata_size_in_byte % SECTOR_SIZE_IN_BYTE == 0 ? 0 : 1);

					if (((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Exists(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA)) {
						Data_Cache_Slot_Type slot = ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Get_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						sim_time_type timestamp = slot.Timestamp;
						NVM::memory_content_type content = slot.Content;
						if (((NVM_Transaction_Flash_WR*)transaction)->DataTimeStamp >= timestamp) {  /* 貌似trace一时半会不会执行 */
							((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Remove_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
						}
					}
					
					/* 主要对在waiting_user_requests_queue_for_dram_free_slot内的事务进行执行 因为本函数是从PHY来 意味着有事务已经得到了服务 释放了缓存阻塞资源 */
					auto user_request = ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].begin();
					while (user_request != ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].end())
					{
						((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->write_to_destage_buffer(*user_request);
						if ((*user_request)->Transaction_list.size() == 0) {
							((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[sharing_id].erase(user_request++);
						} else {
							user_request++;
						}
						//The traffic load on the backend is high and the waiting requests cannot be serviced
						if (((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->back_pressure_buffer_depth[sharing_id] > ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->back_pressure_buffer_max_depth) {
							break;
						}
					}
					...
					{
						std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
						while (!((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Empty())
						{
							DataCacheSlotType evicted_slot = ((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_dirty_slot();
						...
						if (cache_eviction_read_size_in_sectors > 0)
						{
							...
							((Data_Cache_Manager_Flash_Fuzzy*)_my_instance)->service_dram_access_request(read_transfer_info);
						}
					}*/
	...
	void Data_Cache_Manager_Flash_Fuzzy::service_dram_access_request(Memory_Transfer_Info* request_info)
	{
	...
	void Data_Cache_Manager_Flash_Fuzzy::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
	...
	/* zy1024cs */
	bool Data_Cache_Manager_Flash_Fuzzy::Cmp(const User_Request* ur1, const User_Request* ur2){
        if ((*ur1).fuzzy_priority < (*ur2).fuzzy_priority)
            return true;
        else
            return false;
    }
