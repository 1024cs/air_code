...
namespace SSD_Components
{
    ...
    TSU_ZY::TSU_ZY(const sim_object_id_type &id, FTL *ftl, NVM_PHY_ONFI_NVDDR2 *NVMController, unsigned int ChannelCount, unsigned int chip_no_per_channel,
                                unsigned int DieNoPerChip, unsigned int PlaneNoPerDie,
                                sim_time_type WriteReasonableSuspensionTimeForRead,
                                sim_time_type EraseReasonableSuspensionTimeForRead,
                                sim_time_type EraseReasonableSuspensionTimeForWrite,
                                bool EraseSuspensionEnabled, bool ProgramSuspensionEnabled)
        : TSU_Base(id, ftl, NVMController, Flash_Scheduling_Type::TSU_ZY, ChannelCount, chip_no_per_channel, DieNoPerChip, PlaneNoPerDie,
                ...
                EraseSuspensionEnabled, ProgramSuspensionEnabled)
    {
        ...
        // ofs.open("/home/zy/code/AIR/MQSim/tsu_len.csv", std::ios::out|std::ios::app);
    }
    ...
    void TSU_ZY::Start_simulation()
    {
    }

    void TSU_ZY::Validate_simulation_config()
    {
    }

    void TSU_ZY::Execute_simulator_event(MQSimEngine::Sim_Event *event)
    {
    }

    void TSU_ZY::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter &xmlwriter)
    {
    ...
    void TSU_ZY::Schedule()
    {
        ...
        if (opened_scheduling_reqs < 0)
        {
            PRINT_ERROR("TSU_ZY: Illegal status!");
        }
        ...
        // int id = (*transaction_receive_slots.begin())->Stream_id;
        ...
        for (std::list<NVM_Transaction_Flash *>::iterator it = transaction_receive_slots.begin(); it != transaction_receive_slots.end(); it++)
        {
            ...
            case Transaction_Type::READ:
                ...
                case Transaction_Source_Type::GC_WL:
                    ...
                    break;
                default:
                    PRINT_ERROR("TSU_ZY: unknown source type for a read transaction!")
                }
                break;
            case Transaction_Type::WRITE:
                ...
                case Transaction_Source_Type::USERIO:
                    ...
                    // Get_LCM_position(&UserWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]);
                    break;
                case Transaction_Source_Type::MAPPING:
                    MappingWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID].push_back((*it));
                    // Get_LCM_position(&MappingWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]);
                    break;
                case Transaction_Source_Type::GC_WL:
                    GCWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID].push_back((*it));
                    // Get_LCM_position(&GCWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]);
                    break;
                default:
                    PRINT_ERROR("TSU_ZY: unknown source type for a write transaction!")
                }
                break;
    ...
    bool TSU_ZY::service_read_transaction(NVM::FlashMemory::Flash_Chip *chip)
    {
    ...
    bool TSU_ZY::service_write_transaction(NVM::FlashMemory::Flash_Chip *chip)
    {
        Flash_Transaction_Queue *sourceQueue1 = NULL, *sourceQueue2 = NULL;
        
        Get_LCM_position(&UserWriteTRQueue[chip->ChannelID][chip->ChipID]);
    ...
    bool TSU_ZY::service_erase_transaction(NVM::FlashMemory::Flash_Chip *chip)
    {
	...
	int TSU_ZY::Get_busy_channels_number(){
        int busy_number = 0;
		for (flash_channel_ID_type channelID = 0; channelID < channel_count; channelID++){
    ...
    bool TSU_ZY::Cmp(const NVM_Transaction_Flash* tr1, const NVM_Transaction_Flash* tr2){
        int pageID1 = (*tr1).Address.PageID, pageID2 = (*tr2).Address.PageID;
        /* 在src/nvm_chip/flash_memory/Flash_Chip.h处有定义 */
		int latencyType1 = (pageID1 <= 5) ? 0 : ((pageID1 <= 7) ? 1 : (((pageID1 - 8) >> 1) % 3));
        int latencyType2 = (pageID2 <= 5) ? 0 : ((pageID2 <= 7) ? 1 : (((pageID2 - 8) >> 1) % 3));

        if (latencyType1 < latencyType2)
            return true;
        else
            return false;
    }
    ...
    void TSU_ZY::Get_LCM_position(Flash_Transaction_Queue* UserTRQueue){
		if (UserTRQueue->size() <= 1)
			return;

        // if (UserTRQueue->size() > queue_size){
        //     queue_size = UserTRQueue->size();
        //     qq(queue_size);
        // }

        // if (UserTRQueue->size() > 10){
        //     for (Flash_Transaction_Queue::iterator it = UserTRQueue->begin(); it != UserTRQueue->end(); it++){
        //         std::cout<<Get_latencyType((*it)->Address.PageID)<<"-";
        //     }
        //     std::cout<<std::endl;
        // }
        
        // ofs<<UserTRQueue->size()<<std::endl;
        UserTRQueue->sort(Cmp);
        // if (UserTRQueue->size() > 10){
        //     for (Flash_Transaction_Queue::iterator it = UserTRQueue->begin(); it != UserTRQueue->end(); it++){
        //         std::cout<<(*it)->Address.latencyType<<"-";
        //     }
        //     std::cout<<std::endl;
        // }
        ...
        // bool pri = false;
        // if (UserTRQueue->size() > 2) {
        //     auto obj = (*UserTRQueue->begin())->Stream_id;
        //     for (Flash_Transaction_Queue::iterator it = UserTRQueue->begin(); it != UserTRQueue->end(); it++){
        //         // std::cout<<(*it)->Stream_id<<"-";
        //         if (obj != (*it)->Stream_id) {
        //             // zz;
        //             pri = true;
        //             break;
