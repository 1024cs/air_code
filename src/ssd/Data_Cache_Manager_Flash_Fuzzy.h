#ifndef Data_Cache_Manager_Flash_Fuzzy_H
#define Data_Cache_Manager_Flash_Fuzzy_H
...
namespace SSD_Components
{
	...
	Assumed hardware structure:
			...
			|     DRAM Data_Cache_Flash Main Data Space                   |   Back Pressure Space  | ---------->To the flash backend
			 --------------------------------------------------|------------------------
	*/
	class Data_Cache_Manager_Flash_Fuzzy : public Data_Cache_Manager_Base
	{
	public:
		Data_Cache_Manager_Flash_Fuzzy(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
		...
		~Data_Cache_Manager_Flash_Fuzzy();
		void Execute_simulator_event(MQSimEngine::Sim_Event* ev);
		void Setup_triggers();
		void Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats);

		/* zy1024cs */
		sim_time_type* stream_bytes_read;
		sim_time_type* stream_bytes_write;
		int* stream_fuzzy_priority;
		
		static bool Cmp(const User_Request* ur1, const User_Request* ur2);

	private:
		NVM_PHY_ONFI * flash_controller;
		unsigned int capacity_in_bytes, capacity_in_pages;
		unsigned int sector_no_per_page;
		Data_Cache_Flash** per_stream_cache;  /* 最后一个'*'代表Data_Cache_Flash的地址 */
		...
		unsigned int *back_pressure_buffer_depth;  /* '*'代表不同的Cache_Sharing_Mode模式下数组中有1个元素还是多个元素 上面的*含义一致 */
		std::set<LPA_type>* bloom_filter;  /* bloom_filter = new std::set<LPA_type>[stream_count]; */
		sim_time_type bloom_filter_reset_step = 1000000000;  /* 1s */
		sim_time_type next_bloom_filter_reset_milestone = 0;

		static void handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction);
		void service_dram_access_request(Memory_Transfer_Info* request_info);  /* 如果memory_channel_is_busy 则放入dram_execution_queue 否则直接注册事件 */
	};
}

#endif // !Data_Cache_Manager_Flash_Fuzzy_H
