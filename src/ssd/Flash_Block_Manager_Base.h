...
#define All_VALID_PAGE 0x0000000000000000ULL
	...
	*/
	...
	public:
		...
		flash_page_ID_type Current_page_write_index;  /* 理解为当前可以写的页边界 */
		Block_Service_Status Current_status;
		unsigned int Invalid_page_count;
		unsigned int Erase_count;  /* 被用在动态磨损均衡dynamic_wl中 */
		...
		bool Holds_mapping_data = false;   /* 是否用来保存mapping的 */
	...
	public:
		...
		std::multimap<unsigned int, Block_Pool_Slot_Type*> Free_block_pool;  /* 分配新的free资源 其中第一个int为Erase_count */
	...
	public:
		...
		virtual unsigned int Get_pool_size(const NVM::FlashMemory::Physical_Page_Address& plane_address) = 0;  /* return Free_block_pool.size() */
		flash_block_ID_type Get_coldest_block_id(const NVM::FlashMemory::Physical_Page_Address& plane_address);  /* 返回对应地址下plane内部最小erase的block的id */
		unsigned int Get_min_max_erase_difference(const NVM::FlashMemory::Physical_Page_Address& plane_address);  /* 返回对应地址下plane内部最大最小block对应erase的差值 */
		...
		bool Block_has_ongoing_gc_wl(const NVM::FlashMemory::Physical_Page_Address& block_address);  // Checks if there is an ongoing gc for block_address, return plane_record->Blocks[block_address.BlockID].Has_ongoing_gc_wl
		...
		void GC_WL_started(const NVM::FlashMemory::Physical_Page_Address& block_address);  // Updates the block bookkeeping record, Has_ongoing_gc_wl=true
		void GC_WL_finished(const NVM::FlashMemory::Physical_Page_Address& block_address);  // Updates the block bookkeeping record, Has_ongoing_gc_wl=false
		void Read_transaction_issued(const NVM::FlashMemory::Physical_Page_Address& page_address);//Updates the block bookkeeping record, Ongoing_user_read_count++
		void Read_transaction_serviced(const NVM::FlashMemory::Physical_Page_Address& page_address);//Updates the block bookkeeping record, Ongoing_user_read_count--
	...
	protected:
		PlaneBookKeepingType ****plane_manager;//Keeps track of plane block usage information /* plane_manager[channelID][chipID][dieID][planeID] */
