																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	case HostInterface_Types::NVME:
		for (uint16_t cmdid = 0; cmdid < (uint16_t)(0xffffffff); cmdid++)  /* 由于uint16_t的存在 限制0xffffffff 所以只能到达65535 理论上应该是4294967295 */
		{
		...
		case 8:
			...
			break;
		case 9:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_9;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_9;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_9;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_9;
			break;
		case 10:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_10;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_10;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_10;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_10;
			break;
		case 11:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_11;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_11;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_11;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_11;
			break;
		case 12:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_12;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_12;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_12;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_12;
			break;
		case 13:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_13;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_13;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_13;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_13;
			break;
		case 14:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_14;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_14;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_14;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_14;
			break;
		case 15:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_15;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_15;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_15;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_15;
			break;
		case 16:
			nvme_queue_pair.Submission_queue_memory_base_address = SUBMISSION_QUEUE_MEMORY_16;
			nvme_queue_pair.Submission_tail_register_address_on_device = SUBMISSION_QUEUE_REGISTER_16;
			nvme_queue_pair.Completion_queue_memory_base_address = COMPLETION_QUEUE_MEMORY_16;
			nvme_queue_pair.Completion_head_register_address_on_device = COMPLETION_QUEUE_REGISTER_16;
			break;
		default:
			break;
		}
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		sim_time_type device_response_time = Simulator->Time() - request->Enqueue_time;   /* 计算一次用户请求的总访问延迟 */
		sim_time_type request_delay = Simulator->Time() - request->Arrival_time;
		// qq(request->Start_LBA * 512 <<"-"<<device_response_time / 1000);
		// qq(cqe->Command_Identifier);
		...
		while(waiting_requests.size() > 0) {   /* 貌似不会出现 因为提交队列非常大 65535 */
			if (!NVME_SQ_FULL(nvme_queue_pair) && available_command_ids.size() > 0) {  /* nvm队列未满 */
		...
		// qq(Get_STAT_sum_transferred_bytes_read_short_term());
		if (progress >= next_progress_step) {
			std::string progress_bar;
			int barWidth = 100;
			progress_bar += "[";
			...
			std::string obj = progress_bar + " " + std::to_string(progress) + "% progress in " + ID();
			std::cout<<obj<<"\r";
			fflush(stdout);


			next_progress_step += 1;    /* 可以修改进度条的细粒度 */ 
		}
	...
	Submission_Queue_Entry* IO_Flow_Base::NVMe_read_sqe(uint64_t address)
	{
		...
		if (request->Type == Host_IO_Request_Type::READ) {
			...
			sqe->Reserved = request->Arrival_time;  /* zy1024cs */
	...
	void IO_Flow_Base::Submit_io_request(Host_IO_Request* request)
	{
		switch (SSD_device_type) {
			case HostInterface_Types::NVME:
				...
				if (NVME_SQ_FULL(nvme_queue_pair) || available_command_ids.size() == 0) {  /* nvm队列满了 先放在waiting_requests中暂存 */
					...
					// qq(request->Start_LBA * 512<<"-"<<request->Arrival_time<<"-"<<request->Enqueue_time);
					pcie_root_complex->Write_to_device(nvme_queue_pair.Submission_tail_register_address_on_device, nvme_queue_pair.Submission_queue_tail);//Based on NVMe protocol definition, the updated tail pointer should be informed to the device
					// qq(Simulator->Time());
				}
				break;
	...
	{
		if (STAT_serviced_request_count_short_term == 0) {  /* 原始为STAT_serviced_request_count == 0  可能错了 */
			return 0;
		}
	...
	/* zy1024cs */
	uint32_t IO_Flow_Base::Get_STAT_sum_transferred_bytes_read_short_term(){
		return STAT_sum_transferred_bytes_read_short_term;
	}

	uint32_t IO_Flow_Base::Get_STAT_sum_transferred_bytes_write_short_term(){
		return STAT_sum_transferred_bytes_write_short_term;
	}
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		/* zy1024cs 计算trace时间间隔 */
		sim_time_type time_long = last_req_time - first_req_time;
		if (time_long <= 0)
			time_long = 1;
		...
		/* zy1024cs 修改时间间隔 下同 */
		val = std::to_string((double)STAT_generated_request_count / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		// qq(first_req_time<<"-"<<last_req_time);
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Read";
		// val = std::to_string((double)STAT_generated_read_request_count / ((double)Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		val = std::to_string((double)STAT_generated_read_request_count / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Write";
		// val = std::to_string((double)STAT_generated_write_request_count / ((double)Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		val = std::to_string((double)STAT_generated_write_request_count / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Bytes_Transferred_Read";    /* byte */
		...
		attr = "Bandwidth";   /* byte/s */
		// val = std::to_string((double)STAT_transferred_bytes_total / ((double)Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		val = std::to_string((double)STAT_transferred_bytes_total / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Read";
		// val = std::to_string((double)STAT_transferred_bytes_read / ((double)Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF));
		val = std::to_string((double)STAT_transferred_bytes_read / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
		...
		val = std::to_string((double)STAT_transferred_bytes_write / ((double)time_long / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);


		attr = "Device_Response_Time";    /* 转换为us了已经 */
		val = std::to_string(Get_device_response_time());
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Device_Response_Read_Time";
		val = std::to_string((STAT_serviced_read_request_count) == 0 ? 0 : (uint32_t)(STAT_sum_device_response_time_read / STAT_serviced_read_request_count / SIM_TIME_TO_MICROSECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Device_Response_Write_Time";
		val = std::to_string((STAT_serviced_write_request_count) == 0 ? 0 : (uint32_t)(STAT_sum_device_response_time_write / STAT_serviced_write_request_count / SIM_TIME_TO_MICROSECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
