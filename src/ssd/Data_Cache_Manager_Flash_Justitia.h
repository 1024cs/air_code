#ifndef DATA_CACHE_MANAGER_FLASH_Justitia_H
#define DATA_CACHE_MANAGER_FLASH_Justitia_H
...
namespace SSD_Components
{
	...
	Assumed hardware structure:
			...
			|     DRAM Data_Cache_Flash Main Data Space                   |   Back Pressure Space  | ---------->To the flash backend
			 --------------------------------------------------|------------------------
	*/
	class Data_Cache_Manager_Flash_Justitia : public Data_Cache_Manager_Base
	{
	public:
		Data_Cache_Manager_Flash_Justitia(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
		...
		~Data_Cache_Manager_Flash_Justitia();
		void Execute_simulator_event(MQSimEngine::Sim_Event* ev);
		void Setup_triggers();
		void Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats);
	
	/* zy1024cs 注意Justitia有做channel的隔离 但是之前测试基本上都忽略了 */
		int op;
		int *number_DRAM_stream;
		int channel_count;
		int evict_stream_ID;
		int num_request;
		void Justitia();
		int Get_busy_channels_number();
		int UpdateOP(User_Request* user_request);
		void UpdateEID();
		Data_Cache_Slot_Type Evict_one_slot_by_Dop();
...
#endif // !DATA_CACHE_MANAGER_FLASH_Justitia_H
