...
namespace NVM
{
	namespace FlashMemory
	{
		...
		{
			connectedReadyHandlers.push_back(function);  /* 只有src/ssd/NVM_PHY_ONFI_NVDDR2.cpp中handle_ready_signal_from_chip函数 */
		}
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)
		{
			...
			this->idleDieNo++;
			if (idleDieNo == die_no) {   /* 没有一个die处于busy状态 设置chip的状态为IDLE */
			...
			switch (command->CommandCode)   /* 统计每个plane接受读写的次数相关的统计信息 */
			{
				case CMD_READ_PAGE:
			...
			broadcast_ready_signal(command);  /* 只有src/ssd/NVM_PHY_ONFI_NVDDR2.cpp中handle_ready_signal_from_chip函数 */
		}
