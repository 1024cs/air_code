...
namespace Host_Components
{
	struct NVMe_Queue_Pair
	{
		...
		uint16_t Submission_queue_size;  /* 65535 */
		uint64_t Submission_tail_register_address_on_device;  /* src/ssd/Host_Interface_Defs.h内进行注册 */
		uint64_t Submission_queue_memory_base_address;  /* src/host/Host_Defs.h内进行注册 */
		uint16_t Completion_queue_head;
		uint16_t Completion_queue_tail;
		uint16_t Completion_queue_size;
		uint64_t Completion_head_register_address_on_device;  /* src/ssd/Host_Interface_Defs.h内进行注册 */
		uint64_t Completion_queue_memory_base_address;  /* src/host/Host_Defs.h内进行注册 */
	};
...
#define NVME_UPDATE_SQ_TAIL(Q)  Q.Submission_queue_tail++;\
	...
	public:
		...
		virtual void NVMe_consume_io_request(Completion_Queue_Entry*);  /* 完成一个请求 统计相关信息 */
		Submission_Queue_Entry* NVMe_read_sqe(uint64_t address);  /* 从memory中根据address生成一个Submission_Queue_Entry */
		...
		/* zy1024cs */
		sim_time_type first_req_time = 0, last_req_time = 0;  /* 注意对读写的测量可能不准 需要额外来计算 */
		sim_time_type STAT_sum_transferred_bytes_read_short_term = 0, STAT_sum_transferred_bytes_write_short_term = 0;
		uint32_t Get_STAT_sum_transferred_bytes_read_short_term();//in byte/xx, depend on logging_period
		uint32_t Get_STAT_sum_transferred_bytes_write_short_term();//in byte/xx, depend on logging_period
	...
	protected:
		...
		std::set<uint16_t> available_command_ids;  /* 内部初始有65535个可用的数 */
		...
		sim_time_type logging_period;   /* 在ssdconfig.xml中的Host_Parameter_Set的ResponseTime_Logging_Period_Length有设置 一般是1000000000 */
