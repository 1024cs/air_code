		...
		static void Tokenize(const std::string& str, char delimiter, std::vector<std::string>& output_tokens_list)
		{
			...
			while (end < size) {
				if (str[end] == delimiter && start <= end) {
					output_tokens_list.push_back(std::string(str.substr(start, end - start)));  /* 注意这里得到的所有元素的末尾都有, */
					start = end + 1;
				}
				end++;
			}
			...
			// output_tokens_list[1] = output_tokens_list[1].substr(0, 1);  /* 注意这里就把R,和W,后面的,给截掉了 */
		}
